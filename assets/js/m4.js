...
window.TOS_M4 = (function () {
  ...
  function viewStation() {
    ...
    const totalCourses = pkg.courseIds().length;
    const lessonsTotal = pkg.courseIds().reduce((n, id) => n + (COURSE_LIB.find(c=>c.id===id)?.chapters.length || 0), 0);
    ...
    if (profileTab === "info") {
      content = `
        <div class="report-block">
          ...
          <div class="chapter-row"><span style="width:70px;font-size:13px;opacity:.6">入职日期</span><span class="mono" style="font-size:13px">${p.created}</span></div>
        </div>`;
    } else if (profileTab === "todo") {
      content = todos.length
        ? `<div class="report-block">${todos.join("")}</div>`
        : `<div class="quiz-done">今日必修课已全部完成 🎉</div>`;
        ...
        <div class="report-block">
          <div style="display:flex;gap:12px;margin-bottom:12px">
            <div style="flex:1;text-align:center;background:var(--c-block-mint);border-radius:var(--r-lg);padding:12px">
              ...
              <div style="font-size:11px;opacity:.6">完成课程</div>
            </div>
            <div style="flex:1;text-align:center;background:var(--c-block-cream);border-radius:var(--r-lg);padding:12px">
              ...
              <div style="font-size:11px;opacity:.6">完成微课</div>
            </div>
            <div style="flex:1;text-align:center;background:var(--c-block-lilac);border-radius:var(--r-lg);padding:12px">
              ...
              <div style="font-size:11px;opacity:.6">通过演练</div>
            </div>
          </div>
          ...
          ${lessonsDone >= 5 ? '<div class="chapter-row"><span class="chapter-check" style="background:var(--c-block-mint);border:none">🔥</span><span style="font-size:13px">微课达人——累计完成 5 节微课</span></div>' : ""}
          ${missionsDone >= 1 ? '<div class="chapter-row"><span class="chapter-check" style="background:var(--c-block-mint);border:none">⚔️</span><span style="font-size:13px">初战告捷——通过首次情景演练</span></div>' : ""}
        </div>`;
    } else if (profileTab === "growth") {
      content = growthItems.length
        ? `<div class="report-block">${growthItems.map(g => `
        ...
        : `<div class="report-block" style="text-align:center;padding:24px"><p style="font-size:13px;opacity:.5">完成初聊后这里会展示你的成长报告</p></div>`;
    }
    ...
    return `
      <div class="learn-page">
        ...
        <div class="view-tabs" style="margin-top:12px">${tabHtml}</div>
        ${content}
  ...
  function handle(act, el) {
    ...
    switch (act) {
      ...
      case "pf-tab": profileTab = el.dataset.tab; app.innerHTML = viewStation(); startBubbles(); return true;
      case "logout": {
        ...
        localStorage.removeItem("tos_sb_last_" + phoneKey());
        location.hash = "#/login";
        return true;
      }
      case "open-course": {
